class ColorGuessingGame {
    constructor() {
        ...
        this.totalGuesses = parseInt(localStorage.getItem('colorGameTotalGuesses')) || 0;
    ...
    bindEvents() {
        ...
        this.elements.resetStatsBtn.addEventListener('click', () => {
            if (confirm('Bạn có chắc muốn xóa tất cả thống kê?')) {
                this.resetStats();
            }
        });
    ...
    displayTargetColor() {
        ...
        const colorString = `rgba(${Math.round(targetColor.r)}, ${Math.round(targetColor.g)}, ${Math.round(targetColor.b)}, 1)`;
        
        this.elements.targetColor.style.backgroundColor = colorString;
        this.elements.colorCode.textContent = `RGBA(${Math.round(targetColor.r)}, ${Math.round(targetColor.g)}, ${Math.round(targetColor.b)}, 1)`;
    }
    ...
    displayColorOptions() {
        ...
        this.currentColors.forEach((color, index) => {
            ...
            const colorString = `rgba(${Math.round(color.r)}, ${Math.round(color.g)}, ${Math.round(color.b)}, 1)`;
    ...
    selectColor(selectedIndex) {
        ...
        this.totalGuesses++;
        localStorage.setItem('colorGameTotalGuesses', this.totalGuesses.toString());
        this.updateStats();
    ...
    updateStats() {
        try {
            ...
            if (this.totalGuesses > 0) {
                accuracy = Math.round((this.totalCorrect / this.totalGuesses) * 100);
            }
    ...
    resetStats() {
        try {
            ...
            localStorage.removeItem('colorGameTotalCorrect');
            localStorage.removeItem('colorGameTotalGuesses');
